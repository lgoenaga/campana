...
import { deleteAsignarContacto } from "../../routes/asignacioncontacto";
...
const TableAsignarContacto = (props) => {
	const {
		noReg,
		_id,
		polling,
	...
	const borrarAsignarContacto = async () => {
			...
			Swal.fire({
				...
				try {
					if (result.isConfirmed) {
						...
						await deleteAsignarContacto(_id, authheader);
	...
	return (
		<>
			<tr className="fila-contactos">
				...
				<td className="dato-contacto">{voter.firstSurname}</td>
				<td className="dato-contacto">{numberPolling}</td>
				<td className="dato-contacto">
					<Button
						variant="btn btn-outline-danger"
						onClick={borrarAsignarContacto}
...
export default TableAsignarContacto;
