...
import { updateAsignarTestigo, getAsignarTestigo } from "../../routes/asignaciontestigo";

import { AuthHeaders } from "../../components/authheader";
import { MostrarRol } from "../../components/decodec";

export const UpdateAsignarTestigo = () => {
	...
	useEffect(() => {
		if (userRol === "Administrador" || userRol === "Operador") {
			const mostrarlugar = async () => {
				try {
					const { data } = await getAsignarTestigo(id);
					setLugar(data);
				} catch (error) {
	...
	useEffect(() => {
		setValoresForm({
			...
			availablePollingStation: lugar.availablePollingStation,
			unavailablePollingStation: lugar.unavailablePollingStation,
		});
	}, [lugar]);

	const findFormErrors = () => {
		const { pollingStation, address, department, township } = valoresForm;
		const newErrors = {};
			...
			Swal.fire({
				...
				try {
					if (result.isConfirmed) {
						...
						data = await updateAsignarTestigo(id, lugar, authheader);
						Swal.fire({
							icon: "success",
